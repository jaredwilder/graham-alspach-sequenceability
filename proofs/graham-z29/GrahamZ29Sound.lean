...
The ONE soundness theorem for the packed certificate checker in `GrahamZ29Checker.lean`
(ERDŐSFIRE-FERRARI §3.3). This is the real mathematical content: it connects the efficient,
`native_decide`-friendly Boolean pipeline to a faithful, independent Lean transcription of
`erdosfire/round4-paper-graham-z29/main.tex`'s "Statement" section, so that ANY witness
satisfying `checkWitness w = true` yields a genuine proof that `decodeSubset w.subsetMask` is
sequenceable -- proved ONCE, here, regardless of how many witnesses the corpus has (60,134 for
`Z_29`).
-/
...
very last is nonzero. Literal transcription of main.tex's "Statement" section. -/
def Sequences (l : List (ZMod 29)) (A : Finset (ZMod 29)) : Prop :=
  l.Nodup ∧ l.toFinset = A ∧
    (∀ i j, i < l.length → j < l.length → i ≠ j →
      (l.take (i + 1)).sum ≠ (l.take (j + 1)).sum) ∧
    (∀ i, i + 1 < l.length → (l.take (i + 1)).sum ≠ 0)

/-- `A` is sequenceable when some ordering sequences it -- main.tex's Theorem: "Every subset
`A ⊆ Z_29 \ {0}` is sequenceable." -/
def Sequenceable (A : Finset (ZMod 29)) : Prop := ∃ l, Sequences l A
...
theorem partialSums_length (l : List (ZMod 29)) : (partialSums l).length = l.length := by
...
theorem partialSums_getElem (l : List (ZMod 29)) (j : ℕ) (hj : j < l.length) :
...
theorem partialSumsDistinct_sound (l : List (ZMod 29))
...
theorem properPartialSumsNonzero_sound (l : List (ZMod 29))
...
subset it decodes to. This is proved ONCE; every one of the corpus's 60,134 canonical-orbit
witnesses (sizes 21-28) becomes a `native_decide`-checkable instantiation of it, never a
separate theorem. -/
