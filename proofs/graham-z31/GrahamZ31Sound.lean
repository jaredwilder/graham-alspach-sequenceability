...
The ONE soundness theorem for the packed certificate checker in `GrahamZ31Checker.lean`
(ERDŐSFIRE-FERRARI §3.3). Z_31-specific mirror of
`public/proofs/graham-z29/GrahamZ29Sound.lean` (`29 -> 31` throughout; no other change --
none of the lemmas below depend on the modulus being prime or on its concrete value, they are
generic `List`/`ZMod` reasoning instantiated at `ZMod 31` instead of `ZMod 29`). This is the
real mathematical content: it connects the efficient, `native_decide`-friendly Boolean pipeline
to a faithful, independent Lean transcription of the Graham/Alspach rearrangement conjecture's
"Statement" (same statement as `Z_29`'s, instantiated at `Z_31`), so that ANY witness satisfying
`checkWitness w = true` yields a genuine proof that `decodeSubset w.subsetMask` is sequenceable
-- proved ONCE, here, regardless of how many witnesses the corpus has (765,548 for `Z_31`).
-/
...
at `Z_31`. -/
def Sequences (l : List (ZMod 31)) (A : Finset (ZMod 31)) : Prop :=
...
`A ⊆ Z_31 \ {0}` is sequenceable." -/
def Sequenceable (A : Finset (ZMod 31)) : Prop := ∃ l, Sequences l A
...
theorem partialSums_length (l : List (ZMod 31)) : (partialSums l).length = l.length := by
...
theorem partialSums_getElem (l : List (ZMod 31)) (j : ℕ) (hj : j < l.length) :
...
theorem partialSumsDistinct_sound (l : List (ZMod 31))
...
theorem properPartialSumsNonzero_sound (l : List (ZMod 31))
...
subset it decodes to. This is proved ONCE; every one of the corpus's 765,548 canonical-orbit
witnesses (sizes 21-30) becomes a `native_decide`-checkable instantiation of it, never a
separate theorem. -/
