...
The ONE soundness theorem for the packed certificate checker in `GrahamZ41Checker.lean`
(ERDŐSFIRE-FERRARI §3.3). Z_41-specific mirror of
`public/proofs/graham-z37/GrahamZ37Sound.lean` (`37 -> 41` throughout; no other change -- none
of the lemmas below depend on the modulus being prime, on its concrete value, or on the packed
mask's machine-word width (that width concern is entirely inside `GrahamZ41Checker.lean`'s
`decodeSubset`/`PackedWitness`, which this file imports and treats opaquely) -- they are generic
`List`/`ZMod` reasoning instantiated at `ZMod 41` instead of `ZMod 37`. This is the real
...
"Statement" (same statement as `Z_29`'s, `Z_31`'s, and `Z_37`'s, instantiated at `Z_41`), so that
ANY witness satisfying `checkWitness w = true` yields a genuine proof that
`decodeSubset w.subsetMask` is sequenceable -- proved ONCE, here, regardless of how many
witnesses the corpus has (114,998 for `Z_41` sizes 34-40).
-/
...
at `Z_41`. -/
def Sequences (l : List (ZMod 41)) (A : Finset (ZMod 41)) : Prop :=
...
`A ⊆ Z_41 \ {0}` [of the sizes actually certified -- see `GrahamZ41Checker.lean`'s module
docstring: this corpus covers sizes 34-40 only] is sequenceable." -/
def Sequenceable (A : Finset (ZMod 41)) : Prop := ∃ l, Sequences l A
...
theorem partialSums_length (l : List (ZMod 41)) : (partialSums l).length = l.length := by
...
theorem partialSums_getElem (l : List (ZMod 41)) (j : ℕ) (hj : j < l.length) :
...
theorem partialSumsDistinct_sound (l : List (ZMod 41))
...
theorem properPartialSumsNonzero_sound (l : List (ZMod 41))
...
subset it decodes to. This is proved ONCE; every one of the corpus's 114,998 canonical-orbit
witnesses (sizes 34-40) becomes a `native_decide`-checkable instantiation of it, never a
separate theorem. -/
