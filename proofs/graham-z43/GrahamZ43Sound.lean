...
The ONE soundness theorem for the packed certificate checker in `GrahamZ43Checker.lean`
(ERDŐSFIRE-FERRARI §3.3). Z_43-specific mirror of
`public/proofs/graham-z41/GrahamZ41Sound.lean` (`41 -> 43` throughout; no other change -- none
of the lemmas below depend on the modulus being prime, on its concrete value, or on the packed
mask's machine-word width (that width concern is entirely inside `GrahamZ43Checker.lean`'s
`decodeSubset`/`PackedWitness`, which this file imports and treats opaquely) -- they are generic
`List`/`ZMod` reasoning instantiated at `ZMod 43` instead of `ZMod 41`. This is the real
...
`Z_43`), so that ANY witness satisfying `checkWitness w = true` yields a genuine proof that
`decodeSubset w.subsetMask` is sequenceable -- proved ONCE, here, regardless of how many
witnesses the corpus has (sizes 36-42).
-/
...
at `Z_43`. -/
def Sequences (l : List (ZMod 43)) (A : Finset (ZMod 43)) : Prop :=
...
`A ⊆ Z_43 \ {0}` [of the sizes actually certified -- see `GrahamZ43Checker.lean`'s module
docstring: this corpus covers sizes 36-42 only] is sequenceable." -/
def Sequenceable (A : Finset (ZMod 43)) : Prop := ∃ l, Sequences l A
...
theorem partialSums_length (l : List (ZMod 43)) : (partialSums l).length = l.length := by
...
theorem partialSums_getElem (l : List (ZMod 43)) (j : ℕ) (hj : j < l.length) :
...
theorem partialSumsDistinct_sound (l : List (ZMod 43))
...
theorem properPartialSumsNonzero_sound (l : List (ZMod 43))
...
subset it decodes to. This is proved ONCE; every one of the corpus's witnesses (sizes 36-42)
