/-
GrahamZ47Checker.lean

Packed certificate-checker pipeline for the Graham/Alspach rearrangement conjecture in `Z_47`,
per `erdosfire/evidence/round4-discovery/graham-z47/witnesses.tsv` and the ERDŐSFIRE-FERRARI
architecture doc's packed-witness / one-checker / one-soundness-theorem pattern. This is a
Z_47-specific mirror of `public/proofs/graham-z43/GrahamZ43Checker.lean` (`43 -> 47`,
`42 -> 46` throughout for the modulus/domain size) -- no other change. `subsetMask` is `UInt64`
for the same reason it was in the Z_43 checker (not a new consideration here): `Z_47 \ {0}` has
46 elements, which EXCEEDS a `UInt32`'s 32-bit capacity (unlike `Z_29`'s 28 and `Z_31`'s 30), so
`UInt64` (safe up to 63 bits) is required, exactly as it already was for `Z_37`'s 36, `Z_41`'s
40, and `Z_43`'s 42.

This certificate covers sizes 40 through 46 ONLY (NOT the full 21-46 range: sizes 21-39 of Z_47
remain unclassified, judged combinatorially infeasible for one overnight run -- see this
directory's evidence trail, `erdosfire/evidence/round4-discovery/graham-z47/generator-summary.json`,
`minimumSize: 40, maximumSize: 46`). The certificate/checker/soundness-theorem machinery below
...
Rather than formalizing each of the certificate's canonical-orbit witnesses (sizes 40-46)
individually as its own theorem, we formalize ONE generic certificate checker as a pure Lean
function, prove ONE soundness theorem about it (`GrahamZ47Sound.lean`), and then every concrete
...
Statement (same "Statement" as `Z_29`'s, `Z_31`'s, `Z_37`'s, `Z_41`'s, and `Z_43`'s, instantiated
at `Z_47`): for a finite subset `A` of an abelian group, call an ordering `a_1,...,a_m` a
SEQUENCING when the partial sums `s_i = sum_{j=1}^i a_j` are pairwise distinct and `s_i ≠ 0` for
`1 ≤ i < m`. Note the final sum `s_m` is explicitly NOT required to be nonzero (it is forced to
be `0` exactly when `A` is the full nonzero group, since `sum_{x in Z_47 \ {0}} x = 1081 ≡ 0
(mod 47)`) -- pairwise distinctness is still required of the full list of `m` sums, including
`s_m`.
...
Both fields are machine-word-sized: no `List (Fin 47)`, no `Finset` literal, no `List Prop`
...
`erdosfire/evidence/round4-discovery/graham-z47/witnesses.tsv`:
* `subsetMask` -- a 46-bit mask over `{1,...,46}` (bit `i`, `0 ≤ i < 46`, set means residue
  `i + 1` is a member of the subset). Needs `UInt64` (NOT `UInt32` -- 46 > 32, same width
  boundary the Z_37 checker already crossed).
* `ordering` -- the claimed sequencing, packed as raw residues `1..46`, each fitting in a
...
/-- Decode a packed 46-bit mask into the `Finset (ZMod 47)` it represents: bit `i`
(`0 ≤ i < 46`) set means residue `i + 1` is a member. -/
def decodeSubset (mask : UInt64) : Finset (ZMod 47) :=
  ((Finset.range 46).filter (fun i => mask.toNat.testBit i = true)).image
    (fun i => ((i + 1 : ℕ) : ZMod 47))

/-- Decode a packed ordering (raw bytes, each in `1..46`) into a `List (ZMod 47)`. -/
def decodeOrdering (packed : Array UInt8) : List (ZMod 47) :=
  packed.toList.map (fun b => (b.toNat : ZMod 47))

/-- `true` iff `ordering` enumerates exactly the elements of `subset`, each exactly once
(no repeats, no omissions, no extras). -/
def orderingMatchesSubset (subset : Finset (ZMod 47)) (ordering : List (ZMod 47)) : Bool :=
...
def partialSums (ordering : List (ZMod 47)) : List (ZMod 47) :=
...
function every witness in the corpus (sizes 40-46) is checked against via `native_decide`,
