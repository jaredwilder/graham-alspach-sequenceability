...
The ONE soundness theorem for the packed certificate checker in `GrahamZ47Checker.lean`
(ERDŐSFIRE-FERRARI §3.3). Z_47-specific mirror of
`public/proofs/graham-z43/GrahamZ43Sound.lean` (`43 -> 47` throughout; no other change -- none
of the lemmas below depend on the modulus being prime, on its concrete value, or on the packed
mask's machine-word width (that width concern is entirely inside `GrahamZ47Checker.lean`'s
`decodeSubset`/`PackedWitness`, which this file imports and treats opaquely) -- they are generic
`List`/`ZMod` reasoning instantiated at `ZMod 47` instead of `ZMod 43`. This is the real
...
at `Z_47`), so that ANY witness satisfying `checkWitness w = true` yields a genuine proof that
`decodeSubset w.subsetMask` is sequenceable -- proved ONCE, here, regardless of how many
witnesses the corpus has (sizes 40-46).
-/
import GrahamZ47Checker
...
at `Z_47`. -/
def Sequences (l : List (ZMod 47)) (A : Finset (ZMod 47)) : Prop :=
...
`A ⊆ Z_47 \ {0}` [of the sizes actually certified -- see `GrahamZ47Checker.lean`'s module
docstring: this corpus covers sizes 40-46 only] is sequenceable." -/
def Sequenceable (A : Finset (ZMod 47)) : Prop := ∃ l, Sequences l A
...
theorem partialSums_length (l : List (ZMod 47)) : (partialSums l).length = l.length := by
...
theorem partialSums_getElem (l : List (ZMod 47)) (j : ℕ) (hj : j < l.length) :
...
theorem partialSumsDistinct_sound (l : List (ZMod 47))
...
theorem properPartialSumsNonzero_sound (l : List (ZMod 47))
...
subset it decodes to. This is proved ONCE; every one of the corpus's witnesses (sizes 40-46)
