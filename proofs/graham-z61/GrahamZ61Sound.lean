...
The ONE soundness theorem for the packed certificate checker in `GrahamZ61Checker.lean`
(ERDŐSFIRE-FERRARI §3.3). Z_61-specific mirror of
`public/proofs/graham-z47/GrahamZ47Sound.lean` (`47 -> 61`, `46 -> 60` throughout; no other
change -- none of the lemmas below depend on the modulus being prime, on its concrete value, or
on the packed mask's machine-word width (that width concern is entirely inside
`GrahamZ61Checker.lean`'s `decodeSubset`/`PackedWitness`, which this file imports and treats
opaquely) -- they are generic `List`/`ZMod` reasoning instantiated at `ZMod 61` instead of
`ZMod 47`. This is the real mathematical content: it connects the efficient,
`native_decide`-friendly Boolean pipeline to a faithful, independent Lean transcription of the
Graham/Alspach rearrangement conjecture's "Statement" (same statement as `Z_29`'s, `Z_31`'s,
`Z_37`'s, `Z_41`'s, `Z_43`'s, and `Z_47`'s, instantiated at `Z_61`), so that ANY witness
satisfying `checkWitness w = true` yields a genuine proof that `decodeSubset w.subsetMask` is
sequenceable -- proved ONCE, here, regardless of how many witnesses the corpus has (sizes 56-60).
-/
import GrahamZ61Checker
...
at `Z_61`. -/
def Sequences (l : List (ZMod 61)) (A : Finset (ZMod 61)) : Prop :=
...
`A ⊆ Z_61 \ {0}` [of the sizes actually certified -- see `GrahamZ61Checker.lean`'s module
docstring: this corpus covers sizes 56-60 only] is sequenceable." -/
def Sequenceable (A : Finset (ZMod 61)) : Prop := ∃ l, Sequences l A
...
theorem partialSums_length (l : List (ZMod 61)) : (partialSums l).length = l.length := by
...
theorem partialSums_getElem (l : List (ZMod 61)) (j : ℕ) (hj : j < l.length) :
...
theorem partialSumsDistinct_sound (l : List (ZMod 61))
...
theorem properPartialSumsNonzero_sound (l : List (ZMod 61))
...
subset it decodes to. This is proved ONCE; every one of the corpus's witnesses (sizes 56-60)
becomes a `native_decide`-checkable instantiation of it, never a separate theorem. -/
